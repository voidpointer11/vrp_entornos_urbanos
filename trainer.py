...
class StateCritic(nn.Module):
    ...
    def forward(self, static, dynamic):
        ...
        static_hidden = self.static_encoder(static).to(device)
        dynamic_hidden = self.dynamic_encoder(dynamic).to(device)
...
def validate(data_loader, actor, reward_fn, render_fn=None, save_dir='.',
    ...
    actor.train()
    print('Validation set: %d batches, mean reward: %2.4f' %
          (len(data_loader), np.mean(rewards)))
...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    train_loader = DataLoader(train_data, batch_size, True, num_workers=15)
    valid_loader = DataLoader(valid_data, batch_size, False, num_workers=15)
    ...
    for epoch in range(100):
        ...
        # Pone a am en modo de entrenamiento
        actor.train()
        critic.train()
        ...
        if epoch % 10 == 0:
            print('Saving actor model at epoch %d' % epoch)
            torch.save(actor, f'checkpoints/adh_50n_{epoch}_tiempo.pth')
        ...
        if not os.path.exists(stats_path):
            ...
            stats.to_csv(stats_path, index=False, mode='a', header=False)
...
def train_vrp(args):
    ...
    # Determina la carga máxima del vehículo a partir del número de nodos
    LOAD_DICT = {10: 20, 20: 30, 50: 40, 100: 50}

    # Demanda máxima de uno de los nodos
    MAX_DEMAND = 9

    # Tamaño de la entrada estática (coordenadas x,y del depósito y de los nodos)
    STATIC_SIZE = 2 # (x, y)

    # Tamaño de la entrada dinámica (carga del vehículo y demanda de los nodos)
    DYNAMIC_SIZE = 2 # load, demanddevice = torch.device('cpu')

    # Carga máxima del vehículo basada en el diccionario
    max_load = LOAD_DICT[args.num_nodes]

    train_data = VehicleRoutingDataset(num_samples=args.train_size,
