...
class Attention(nn.Module):
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden):
        ...
        static_hidden = static_hidden.to(device)
        dynamic_hidden = dynamic_hidden.to(device)
        hidden = hidden.to(device)
        hidden = torch.cat((static_hidden, dynamic_hidden, hidden), 1)


        # Broadcast some dimensions so we can do batch-matrix-multiply
        v = self.v.expand(batch_size, 1, hidden_size).to(device)
        W = self.W.expand(batch_size, hidden_size, -1).to(device)
...
class Pointer(nn.Module):
    ...
    def __init__(self, hidden_size, num_layers=1, dropout=0.2):
        ...
        self.gru = nn.GRU(hidden_size, hidden_size, num_layers,
                          batch_first=True,
                          dropout=dropout if num_layers > 1 else 0).to(device)
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden, last_hh):
        ...
        v = self.v.expand(static_hidden.size(0), -1, -1).to(device)
        W = self.W.expand(static_hidden.size(0), -1, -1).to(device)
...
class DRL4TSP(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size,
        ...
        self.static_encoder = Encoder(static_size, hidden_size).to(device)
        self.dynamic_encoder = Encoder(dynamic_size, hidden_size).to(device)
        self.decoder = Encoder(static_size, hidden_size).to(device)
        self.pointer = Pointer(hidden_size, num_layers, dropout).to(device)
