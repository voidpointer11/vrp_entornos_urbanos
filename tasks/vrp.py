...
class VehicleRoutingDataset(Dataset):
    def __init__(self, num_samples, input_size, max_load=20, max_demand=9,
        ...
        locations = self.load_locations(num_samples,input_size)
    ...
    def load_locations(self, num_samples, input_size):
        # Get the locations of a given city
        df_locations = pd.read_parquet('graphs/adh_nodes.parquet')

        # Format the coordinates to match the expected input
        coords = torch.tensor(df_locations[['x', 'y']].values, dtype=torch.float32)
        idx = torch.randint(0, len(coords), (num_samples, input_size + 1))
        batch = coords[idx]

        return batch.permute(0, 2, 1)  # (num_samples, 2, input_size + 1)
    ...
    def update_dynamic(self, dynamic, chosen_idx):
        ...
        # Get the current load and demand for each sample
        all_loads = all_loads.to(chosen_idx.device)
        all_demands = all_demands.to(chosen_idx.device)
...
def reward(static, tour_indices):
    ...
    static.data = static.data.to(idx.device)
    ...
    # y is the full tour, including the depot at the start and end
    y = torch.cat((start, tour, start), dim=1)


    paths = y
    batch_size, n_sample, _ = y.size()

    # Paso 1: Convertir coordenadas del camino a índices de nodos
    # Reshape para poder usar cdist eficientemente
    paths_reshaped = paths.view(batch_size * n_sample, 2)

    # Encontrar el nodo más cercano para cada punto del camino
    # usando las coordenadas de referencia
    distances_to_nodes = torch.cdist(paths_reshaped, node_coords)
    node_indices_flat = torch.argmin(distances_to_nodes, dim=1)

    # Reshape de vuelta a la forma original
    node_indices = node_indices_flat.view(batch_size, n_sample)


    # Paso 2: Obtener nodos consecutivos para calcular distancias
    current_nodes = node_indices[:, :-1]  # (batch, n_sample-1)
    next_nodes = node_indices[:, 1:]      # (batch, n_sample-1)

    
    # Paso 3: Indexar la matriz de distancias de forma eficiente
    # Aplanar para usar indexing avanzado
    current_flat = current_nodes.flatten()
    next_flat = next_nodes.flatten()
    
    # Obtener las distancias entre nodos consecutivos
    segment_distances_flat = distance_matrix[current_flat, next_flat]
    
    # Reshape y sumar para obtener la distancia total de cada camino
    segment_distances = segment_distances_flat.view(batch_size, n_sample-1)
    total_distances = torch.sum(segment_distances, dim=1)
...
def load_distance_matrix():
    nodes = pd.read_parquet('graphs/adh_nodes.parquet')
    ids = torch.tensor(nodes['id'].values, dtype=torch.long)
    id2idx = {id: idx for idx, id in enumerate(ids.tolist())}

    coords = torch.tensor(nodes[['x', 'y']].values, dtype=torch.float32)

    distances = pd.read_parquet('graphs/adh_distances.parquet')

    distances['idx1'] = distances.id_1.map(id2idx)
    distances['idx2'] = distances.id_2.map(id2idx)

    distances = distances.dropna(subset=['idx1', 'idx2'])

    idx1 = torch.tensor(distances['idx1'].values, dtype=torch.long)
    idx2 = torch.tensor(distances['idx2'].values, dtype=torch.long)
    dist = torch.tensor(distances['distance'].values, dtype=torch.float32)

    N = len(nodes)
    D = torch.full((N, N), float('inf'))            # inicializa a ∞
    D[idx1, idx2] = dist
    D[idx2, idx1] = dist            # si es simétrica


    return coords.to(device), D.to(device)
...
node_coords, distance_matrix = load_distance_matrix()
